  ...
  context 'when scheduled' do
    context 'scheduled before current time' do
      it 'creates a new job' do
        ...
        expect(run_time).to be < Time.now + 1.day
        expect(run_time).to be >= s.start_today + 1.day
        expect(run_time).to be <= s.stop_today + 1.day
      end
    end
    ...
    context 'scheduled after current time' do
      it 'creates a new job' do
        ...
        run_time = SwipeJob.last.scheduled_at
        # expect(run_time).to be < Time.now
    ...
    context 'in time range' do
      ...
      it 'keeps on creating jobs' do
        ...
        run_time = SwipeJob.last.scheduled_at
        expect(run_time).to be >= s1_start_time + 1.day
        expect(run_time).to be <= s1_stop_time + 1.day
        ...
        expect(run_time).to be >= s1_start_time + 2.day
        expect(run_time).to be <= s1_stop_time + 2.day

        Timecop.freeze(Time.now + 1.day)
        s1.run
        run_time = SwipeJob.last.scheduled_at
        expect(SwipeJob.count).to eq(3)
        expect(run_time).to be >= s1_start_time + 3.day
        expect(run_time).to be <= s1_stop_time + 3.day
      end
    end
    ...
    context 'before todays time range' do
      it 'create a job for today' do
        ...
        Timecop.freeze(Time.now + 6.hours)
        expect{s.run}.to change{SwipeJob.count}.by(0)
        expect{s.run}.to change{SwipeJob.count}.by(0)
    ...
    context 'after todays time range' do
      it 'creates job for next sesssion' do
        ...
        expect{s.run}.to change{SwipeJob.count}.by(0)
        expect{s.run}.to change{SwipeJob.count}.by(0)
        expect(SwipeJob.last.scheduled_at).to be >= Time.zone.now.change({ hour: 19, min: 00}) + 1.day
        expect(SwipeJob.last.scheduled_at).to be <= Time.zone.now.change({ hour: 23, min: 00}) + 1.day
      end
    end
  end
