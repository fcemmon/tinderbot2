class SwipeJob < ApplicationRecord
  ...
  def stopped_more_than_one_min_ago?
    (completed_at && status == "completed" && (Time.now - completed_at) > 60) ||
      (failed_at && status == "failed" && (Time.now - failed_at) > 60) ||
        (failed_at && status == "cancelled" && (Time.now - failed_at) > 60)
  end
  ...
  #   unless self.scheduled_at > Time.now
  ...
  def cancel!
    if status == "scheduled"
      ...
      self.update!( status: 'cancelled', failed_at: Time.now)
    end
  end
