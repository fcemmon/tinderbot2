class Schedule < ApplicationRecord
  ...
  def jobs_created_today
    swipe_jobs.where("created_at > ?", 1.day.ago)
  end

  def can_schedule_today?
    return true if Time.now < start_today
    start_today <= Time.zone.now && Time.zone.now <= stop_today
  end

  def start_today
    ct = Time.zone.now
    start_time.change({year: ct.year, month: ct.month, day: ct.day })
  end

  def stop_today
    ct = Time.zone.now
    stop_time.change({year: ct.year, month: ct.month, day: ct.day })
  end
    ...
    Schedule.transaction do
      ...
      applicable_accounts.find_each do |account|
        ...
        if run_now || job_type_status_check?
          ...
          scheduled_at += 1.day if !can_schedule_today?
        end
      ...
      update!(run_at: Time.now)
    end
  end

  def current_schedule_start
    Time.now >= stop_today ? start_today + 1.day : start_today
  end

  def current_schedule_stop
    Time.now >= stop_today ? stop_today + 1.day : stop_today
  end
