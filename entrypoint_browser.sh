...
if [ "X${DESKTOP_VNC_PASSWORD}" != "X" ] ; then
	echo "init password"
	x11vnc -shared -storepasswd ${DESKTOP_VNC_PASSWORD:-password} ~/.vnc/passwd && chmod 0600 ~/.vnc/passwd
...
export FD_GEOM=${DESKTOP_SIZE}		# To init a screen display when using Xvfb
{
  while [ 1 ] ; do
    # figlet "x11vnc"
    /usr/bin/xinit /home/user/.xinitrc -- /usr/bin/Xvfb :20 -screen 0 ${DESKTOP_SIZE}x24 -cc 4 -nolisten tcp &
    sleep 2
    x11vnc -shared -nopw -display :20 -forever -repeat ${DESKTOP_VNC_PARAMS}
    sleep 1
    # xhost +
    # startx
  done
...
if [ $# -ne 0 ] ; then
    ...
    DISPLAY=:20.0 node launch_profile.js $1
else
	tail -f /dev/null
fi
